  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newUserMapping = {
        ...
        airtableUrl: `https://airtable.com/embed/app${username}${Date.now()}/tblReservas?backgroundColor=orange&view=${restaurantConfig?.airtableViews.today || 'viwReservasHoy'}`,
        retellConfig: restaurantConfig ? {
          agentId: `agent_${username}_${Date.now()}`,
          apiKey: `retell_key_${username}_${Math.random().toString(36).substring(2, 8)}`,
          voiceId: restaurantConfig.voiceSettings.voiceId,
          language: restaurantConfig.voiceSettings.language
        } : undefined,
        twilioConfig: {
                    ...
                    <div className="space-y-2">
                      ...
                      {formData.restaurantType && (
                        <p className="text-sm text-gray-600">
                          {getRestaurantTypeConfig(formData.restaurantType)?.description}
                        </p>
                      )}
                    </div>
              ...
              >
                ...
                Ver Flujo Recomendado
                  ...
                  <Button 
                    variant="outline" 
                    className="w-full"
                    onClick={() => router.push('/admin/auto-table-generator')}
                  >
